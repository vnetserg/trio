...
fn main() {
    ...
    let mut first_lines = HashSet::with_hasher(RandomState::<city::Hash64>::new());
    for line in first_in.split('\n' as u8).map(|line| line.unwrap()) {
        first_lines.insert(line);
    }

    let mut common_lines = HashSet::with_hasher(RandomState::<city::Hash64>::new());
